...
PROJECT_ID = "deb2-200016"
GCS_BUCKET = "deb2-bootcamp-200016"
BIGQUERY_DATASET = "networkrail"
KEYFILE_FOR_GCS = f"{DAGS_FOLDER}/deb2-200016-deb2-loading-files-to-gcs-3f9e51d411b2.json"
KEYFILE_FOR_GCS_TO_BIGQUERY = f"{DAGS_FOLDER}/deb2-200016-deb2-gcs-to-bigquery-d270ccbb52c3.json"


def _extract_data(**context):
    ds = context["data_interval_start"].to_date_string() #current time in airflow
    ...
    #Query movement note: data ตอนเทียบ where ต้องใส่ ''
    sql = f"""
        select * from {DATA}
    ...
    if rows:
        with open(f"{DAGS_FOLDER}/{DATA}-{ds}.csv", "w") as f:
            ...
            writer.writerows(rows) # read all rows
        return "load_data_to_gcs" # By using BranchPythonOperator when task is complete, move to the branch "load_data_to_gcs"
    else:
        return "do_nothing" # if there is no data, then do nothing
...
def _load_data_to_gcs(**context):
    ...
    bucket_name = "deb2-bootcamp-200016"
...
with DAG(
    ...
    # Task dependencies
    start >> extract_data >> load_data_to_gcs >> load_data_from_gcs_to_bigquery >> end #if there is a row extracted
    extract_data >> do_nothing >> end #if there no row extracted